...
void bt_chk_and_disp(void)
{
	...
	if(bt_connected)
	{
		...
		show_modeled_on(SOURCE_SELECT_BT);
	}
	else
	{
	    if(!isDisp)
...
void aux_set_connect_state(bool state)
{
    ...
    //自动模式切换处理
    if(ui_source_select != SOURCE_SELECT_AUX && state)
    {//非LINEIN状态
        //切换到LINEIN状态
        ui_handle_mode(SOURCE_SELECT_AUX, false);
    }
}
...
int bt_cmd_check(char *buf_recv)
{
    ...
    if(index > AT_START && index < AT_MAX)
    {
        ...
        else if (index == UI_CMD_BLUETOOTH_CONNECT)
        {
            //设备连接
            //printf("BT conneted\n");
            //cmd.arg2 = true;
        ...
        else if (index == UI_CMD_BLUETOOTH_DISCONNECT)
        {
            //设备断开连接
            //bt_wait_cnt = 0;
            //bt_wait_flag = false;
...
void ui_handle_tone_finish(void)
{
    ...
    else if(ui_source_select == SOURCE_SELECT_BT || ui_source_select == SOURCE_SELECT_AUX)
    {
...
void ui_handle_vol_set(int vol)
{
    ...
    if(vol >= 0 && vol <= 50)
    {
		...
		else if (ui_source_select == SOURCE_SELECT_BT)
		{
			...
			ui_source_select == SOURCE_SELECT_AUX)
		{
...
static void ui_process_vol_dec(void)
{
	...
	{	
		if (bt_mix_vol >= VOL_STEP) 
			bt_mix_vol -= VOL_STEP;
		else   
			bt_mix_vol = 0;
		
		mix_vol = bt_mix_vol * (100.0f/32) ;
...
int sl_ui_handle_cec_volume_key_up(struct ui_cmd_s *cmd)
{
#ifdef SL_UI_DBG
	printf("%s %d\n", __func__, __LINE__);
#endif
...
int sl_ui_handle_cec_volume_key_down(struct ui_cmd_s *cmd)
{
#if 1//def SL_UI_DBG
	printf("%s %d\n", __func__, __LINE__);
#endif
...
int sl_ui_handle_cec_volume_key_release(struct ui_cmd_s *cmd)
{
#if 1//def SL_UI_DBG
	printf("%s %d\n", __func__, __LINE__);
#endif
...
int sl_ui_handle_cec_mute_key(struct ui_cmd_s *cmd)
{
#ifdef SL_UI_DBG
	printf("%s %d\n", __func__, __LINE__);
#endif
...
int action_hdmi_soundbar_adj_tv_vol(void)
{
	int cur_volum;
...
int action_hdmi_soundbar_mute_tv(void)
{
#ifdef SL_UI_DBG
	printf("%s %d\n", __func__, __LINE__);
#endif
...
int action_hdmi_soundbar_unmute_tv(void)
{
#ifdef SL_UI_DBG
	printf("%s %d\n", __func__, __LINE__);
#endif
