...
WDOG_ID wdtimer_hdmi_detect = NULL;
...
extern int yunos_bsp_flash_page_program(int addr, int length, int buf);
    ...
    while (1)
    {
        if(ui_power == POWER_ON)
        {
            ...
            if (ret > 0)
            {
                ...
                {
                    action_hdmi_standby();
					player_process_cmd(NP_CMD_VOLUME_SET, NULL,25, NULL, NULL);
                    printf("%s:AT+IDLE.\n", __func__);
                }
...
void ui_handle_power(void)
{
    ...
    if (ui_power == POWER_ON)
    {
		...
		{
			printf("read from flash success ! \n");
			if (ui_source_select_temp > SOURCE_SELECT_START &&
				ui_source_select_temp < SOURCE_SELECT_END)
			{
				ui_source_select = ui_source_select_temp - 1;
			}
		    else
		    {
		...
		if(wdtimer_hdmi_detect == NULL)
        {
            wdtimer_hdmi_detect = wd_create();
        }
...
{
    bool ret = false;
//    int buf[0x10] = {0x01, (int)set_vol, ui_source_select_temp};
    int buf[0x10] = {0x01, (int)mix_vol, ui_source_select_temp};
    int length = sizeof(buf);

    printf("write flash data %d %d", (int)mix_vol, ui_source_select_temp);
    yunos_bsp_flash_erase(FLASH_USER_SPACE_ADDR, 0xd8);
    int i = 0;
    for(i=0;i<3;++i)
    {
        if(yunos_bsp_flash_page_program(FLASH_USER_SPACE_ADDR, length, (int)buf) == 0)
        {//写锟斤拷晒锟?            break;
        }
    }
	...
	if (ret)
	{
		printf("update_params_2_flash success \n");
	}
	else
	{
		printf("update_params_2_flash fail \n");
	}
    return ret;
}
...
static bool update_params_from_flash(void)
{
    ...
    for(i=0;i<3;i++)
    {
	    if(yunos_bsp_flash_read(FLASH_USER_SPACE_ADDR, length, (unsigned char *)buf) != -1)
	    {
	    	printf("yunos bsp flash read success val !\n");
			break;
		}
		else
		{
	    	printf("yunos bsp flash read failed !\n");
		}
    }
	printf("-----> flash read buf %d %d %d \n", buf[0], buf[1], buf[2]);
    if(buf[0] == 1)
    {
    	if (buf[1] >= 0 && buf[1] <= 100)
    	{
			mix_vol = buf[1];
			set_vol = (unsigned char)(mix_vol * (16.0f/100));
    	}
		else
		{
			set_vol = 16;
        ...
        ui_source_select_temp = (int)buf[2];

		printf("-----> read flash %d %d %d\n", set_vol, (int)mix_vol, ui_source_select_temp);
        ret = true;
    }
    else
    {
