...
//static int key_value_tbl[] = {0x7, 0x168, 0x72e, 0x453};
...
static void Set_IRKey_Value(void)
{
        switch(irkey_detect)
        {
            case IR_MUTE:
                  key_priv.key_num = CMD_IR_MUTE;
		          key_priv.ch_num = 0;
                  break;
            case IR_EQ1:
                  key_priv.key_num = CMD_IR_EQ1;
		          key_priv.ch_num = 0;
                  break;
            case IR_EQ2:
                  key_priv.key_num = CMD_IR_EQ2;
		          key_priv.ch_num = 0;
                  break;
            case IR_EQ3:
                  key_priv.key_num = CMD_IR_EQ3;
		          key_priv.ch_num = 0;
                  break;
            case IR_LINE:
                  key_priv.key_num = CMD_IR_LINE;
		          key_priv.ch_num = 0;
                  break;
            case IR_HDMI:
                  key_priv.key_num = CMD_IR_HDMI;
		          key_priv.ch_num = 0;
                  break;
            case IR_AUX:
                  key_priv.key_num = CMD_IR_AUX;
		          key_priv.ch_num = 0;
                  break;
            case IR_VOL_UP:
                  key_priv.key_num = CMD_IR_VOL_UP;
		          key_priv.ch_num = 0;
                  break;
            case IR_VOL_DOWN:
                  key_priv.key_num = CMD_IR_VOL_DOWN;
		          key_priv.ch_num = 0;
                  break;
            case KEY_POWER:
                  key_priv.key_num = CMD_KEY_POWER;
		          key_priv.ch_num = 0;
                  break;
            case IR_BT:
                  key_priv.key_num = CMD_IR_BT;
		          key_priv.ch_num = 0;
                  break;
			case IR_PLAY:
				  key_priv.key_num = CMD_IR_PLAY;
		          key_priv.ch_num = 0;
				  break;
        }
 }

 static int Key_detect_irpower_pin(void)
 {
     if(irkey_detect != -1)//||(HDMI_Detect()))
     {
        return 1;
     }
     else
     ...
     #if 0
	 uint32_t keyvalue=0;
	 zhuque_bsp_gpio_set_mode(POWER_SOURCE_BUTTON, GPIO_IN, PULLING_HIGH);
	 zhuque_bsp_gpio_get_value(POWER_SOURCE_BUTTON, &keyvalue);
	 //printf("keyvalue : %d \n", keyvalue);
	 if(keyvalue==0)
		 return 1;
	 else
...
static int key_detect_channel(void)
{
    ...
    //检查是有有按键
    m = KEY_RANGE(buf[key_priv.ch_num], key_value_tbl[key_priv.key_num]) | Key_detect_irpower_pin();
    if (m)
    {   //有按键
        return 0;
    }
...
static int key_find_channel(void)
{
	...
	if(Key_detect_irpower_pin())
	{
		/*if(HDMI_Detect())
		{
         ...
         Set_IRKey_Value();
		//key_priv.key_num = 0x7; //CMD_UI_PWR
		//key_priv.ch_num = 0;
		//printf("%s:Detected Powerkey !\n", __func__);
	}
	else
	{
