 ...
 *
 * Parameters:
 *    增益值 取值范围【0，31】, 对应【+6，-25db], step 为1db,默认为6--0db
 *
 * Returned Value:
 *
 * Assumptions:
...
void set_hdmi_gain(int gain)
{
    #if 1
	unsigned long  value = 0;
    value = REG32(KSEG1(GOL_GAIN_ADDR));
	value = (unsigned long)gain;
	REG32(KSEG1(GOL_GAIN_ADDR)) = value;
	printf("GOL_GAIN_ADDR = %d\n",value);

	value = REG32(KSEG1(GOR_GAIN_ADDR));
	value = (unsigned long)gain;
	REG32(KSEG1(GOR_GAIN_ADDR)) = value;
	printf("GOR_GAIN_ADDR = %d\n",value);
    #endif
}
/****************************************************************************
 * Name: setADC0Vol
