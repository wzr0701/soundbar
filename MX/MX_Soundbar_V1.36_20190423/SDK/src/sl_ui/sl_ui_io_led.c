 ...
 void padmux_init(void)
{
	...
	zhuque_bsp_gpio_set_mode(PT2386_CTL_PIN, GPIO_OUT, PUSH_PULL);
	zhuque_bsp_gpio_set_value(PT2386_CTL_PIN, GPIO_VALUE_LOW);
...
void switch_4052_function(int function)
{
	...
	switch (function)
	{
		case FM_4052:
			...
			printf("%s:choose FM.\n",__func__);
			break;
		case AUX_4052://RCA        SW1:B --- 1   SW2:A --- 0
			sw1 = 0;
			sw2 = 1;
			printf("%s:choose AUX.\n",__func__);
			break;
		case RCA_4052://AUX        SW1:B --- 0    SW2:A --- 1
			sw1 = 1;
			sw2 = 0;
			printf("%s:choose RCA.\n",__func__);
			break;
 ...
 *
 ****************************************************************************/
void pt2386_switch_ctrl(bool pt2386_open)
{
    const int pin = PT2386_CTL_PIN;
    zhuque_bsp_gpio_set_mode(pin, GPIO_OUT, PUSH_PULL);

    zhuque_bsp_gpio_set_value(pin, pt2386_open?GPIO_VALUE_HIGH:GPIO_VALUE_LOW);

	printf("pt2386_switch_ctrl === %d\r\n",pt2386_open);
}
