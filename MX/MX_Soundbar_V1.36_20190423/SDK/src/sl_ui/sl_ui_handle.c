...
void ui_handle_power(int power_on_off)
{
	...
	if (ui_power == POWER_ON)
	{
		...
		//usleep(500000);
		dis_play_update();
		g_FMType_RX = fm_rx_ID();
		printf("%s:g_FMType_RX = %d.\n",__func__,g_FMType_RX);
		//rda5807_read_chipid();
...
IR_VALUE_S IR_CMD[] =
{
	...
	{"AT+IRf60212\r\n",UI_CMD_GO_TO_BT}, 	 //BT MODE
	{"AT+IRf60213\r\n",UI_CMD_GO_TO_USB}, 	 //USB MODE
	{"AT+IRf60214\r\n",UI_CMD_GO_TO_HDMI}, 	 //HDMI MODE
	{"AT+IRf6021a\r\n",UI_CMD_GO_TO_COA}, 	 //COA MODE
	{"AT+IRf6021b\r\n",UI_CMD_GO_TO_SPDIF}, 	 //OPT MODE
	{"AT+IRf6021c\r\n",UI_CMD_GO_TO_AUX1}, 	 //AUX1 MODE
	{"AT+IRf6021d\r\n",UI_CMD_GO_TO_AUX2}, 	 //AUX2 MODE
	{"AT+IRf6021e\r\n",UI_CMD_EQ_SELECT}, 	 //EQ SELECT
	{"AT+IRf6021f\r\n",UI_CMD_BT_PAIR}, 	 //PAIR
	{"AT+IRf60250\r\n",UI_CMD_SYS_RESET}, 	 //RESET
	{"AT+IRf60251\r\n",UI_CMD_NULL}, 	 //REPEAT
...
ui_cmd_t ir_cmd_translate(char *cmd)
{
	ui_cmd_t cmd_t;
    int i;
	cmd_t.cmd = UI_CMD_NULL;
    ...
    {
        //if(strstr(cmd,IR_CMD[i].ir_value) != NULL)
        if(str_cmp_80(cmd,IR_CMD[i].ir_value))
        {
			//printf("cmd = %d\n",IR_CMD[i].cmd);
			cmd_t.cmd = IR_CMD[i].cmd;
			break;
        }
    }
...
ui_cmd_t bt_cmd_check(char *buf_recv)
{
    ui_cmd_t cmd = {0};
    cmd.cmd = UI_CMD_NULL;
    int value;
	...
	if(index > AT_START && index < AT_MAX)
    {
		...
		else if (index == AT_WOOFER_CONNECTED)
        {//设备连接
        	printf("woofer conneted\n");
			woofer_connected         = true;
        }
        else if (index == AT_WOOFER_DISCONNECTED)
        {//设备断开连接
			woofer_connected         = false;
        	printf("woofer disconneted\n");
        } 
		else if(index == AT_SET_VOL)
        {
...
ui_cmd_t ir_cmd_check(char *buf_recv)
{
    ...
    while (1)
    {
        if(ui_power == POWER_ON)
        {
            ...
            if (ret > 0)
            {
                ...
                cmd = bt_cmd_check(buf_recv);
                if(cmd.cmd == UI_CMD_NULL)
                {
                    cmd = ir_cmd_check(buf_recv);
                }
